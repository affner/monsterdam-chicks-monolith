  ...
  return (
    <div>
      <h2 id="state-heading" data-cy="StateHeading">
        <Translate contentKey="monsterdamChicksApp.state.home.title">States</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="monsterdamChicksApp.state.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/state/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="monsterdamChicksApp.state.home.createLabel">Create new State</Translate>
          </Link>
        </div>
      </h2>
        ...
        {stateList && stateList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="monsterdamChicksApp.state.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('stateName')}>
                  <Translate contentKey="monsterdamChicksApp.state.stateName">State Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('stateName')} />
                </th>
                <th className="hand" onClick={sort('isoCode')}>
                  <Translate contentKey="monsterdamChicksApp.state.isoCode">Iso Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('isoCode')} />
                </th>
                <th className="hand" onClick={sort('createdDate')}>
                  <Translate contentKey="monsterdamChicksApp.state.createdDate">Created Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('createdDate')} />
                </th>
                <th className="hand" onClick={sort('lastModifiedDate')}>
                  <Translate contentKey="monsterdamChicksApp.state.lastModifiedDate">Last Modified Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('lastModifiedDate')} />
                </th>
                <th className="hand" onClick={sort('createdBy')}>
                  <Translate contentKey="monsterdamChicksApp.state.createdBy">Created By</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('createdBy')} />
                </th>
                <th className="hand" onClick={sort('lastModifiedBy')}>
                  <Translate contentKey="monsterdamChicksApp.state.lastModifiedBy">Last Modified By</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('lastModifiedBy')} />
                </th>
                <th className="hand" onClick={sort('deletedDate')}>
                  <Translate contentKey="monsterdamChicksApp.state.deletedDate">Deleted Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('deletedDate')} />
                </th>
                <th>
                  <Translate contentKey="monsterdamChicksApp.state.country">Country</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {stateList.map((state, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{state.deletedDate ? <TextFormat type="date" value={state.deletedDate} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="monsterdamChicksApp.state.home.notFound">No States found</Translate>
            </div>
          )
        )}
      </div>
