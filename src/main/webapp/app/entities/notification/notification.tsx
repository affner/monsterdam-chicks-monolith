  ...
  return (
    <div>
      <h2 id="notification-heading" data-cy="NotificationHeading">
        <Translate contentKey="monsterdamChicksApp.notification.home.title">Notifications</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="monsterdamChicksApp.notification.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/notification/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="monsterdamChicksApp.notification.home.createLabel">Create new Notification</Translate>
          </Link>
        </div>
      </h2>
        ...
        {notificationList && notificationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="monsterdamChicksApp.notification.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('notificationKind')}>
                  <Translate contentKey="monsterdamChicksApp.notification.notificationKind">Notification Kind</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('notificationKind')} />
                </th>
                ...
                <th className="hand" onClick={sort('commentedUserId')}>
                  <Translate contentKey="monsterdamChicksApp.notification.commentedUserId">Commented User Id</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('commentedUserId')} />
                </th>
                <th className="hand" onClick={sort('messagedUserId')}>
                  <Translate contentKey="monsterdamChicksApp.notification.messagedUserId">Messaged User Id</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('messagedUserId')} />
                </th>
                <th className="hand" onClick={sort('mentionerIdInPost')}>
                  <Translate contentKey="monsterdamChicksApp.notification.mentionerIdInPost">Mentioner Id In Post</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('mentionerIdInPost')} />
                </th>
                <th className="hand" onClick={sort('mentionerIdInComment')}>
                  <Translate contentKey="monsterdamChicksApp.notification.mentionerIdInComment">Mentioner Id In Comment</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('mentionerIdInComment')} />
                </th>
                <th>
                  <Translate contentKey="monsterdamChicksApp.notification.comment">Comment</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="monsterdamChicksApp.notification.post">Post</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="monsterdamChicksApp.notification.message">Message</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {notificationList.map((notification, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`monsterdamChicksApp.NotificationKind.${notification.notificationKind}`} />
                  </td>
                  <td>
                  ...
                  <td>{notification.commentedUserId}</td>
                  <td>{notification.messagedUserId}</td>
                  <td>{notification.mentionerIdInPost}</td>
                  <td>{notification.mentionerIdInComment}</td>
                  <td>
                    {notification.comment ? <Link to={`/post-comment/${notification.comment.id}`}>{notification.comment.id}</Link> : ''}
                  </td>
                  <td>{notification.post ? <Link to={`/post-feed/${notification.post.id}`}>{notification.post.id}</Link> : ''}</td>
                  <td>
                    {notification.message ? <Link to={`/direct-message/${notification.message.id}`}>{notification.message.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="monsterdamChicksApp.notification.home.notFound">No Notifications found</Translate>
            </div>
          )
        )}
      </div>
