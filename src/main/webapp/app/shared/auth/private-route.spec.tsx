...
describe('hasAnyAuthority', () => {
  ...
  it('Should return true when authorities is valid and hasAnyAuthorities contains an authority', () => {
    ...
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], [AUTHORITIES.VIEWER])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], [AUTHORITIES.VIEWER, AUTHORITIES.ADMIN])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], [AUTHORITIES.VIEWER, 'ROLEADMIN'])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], [AUTHORITIES.ADMIN])).toEqual(true);
  });

  it('Should return false when authorities is valid and hasAnyAuthorities does not contain an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.VIEWER], [AUTHORITIES.ADMIN])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], ['ROLE_VIEWERSS'])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.VIEWER, AUTHORITIES.ADMIN], ['ROLEUSER', 'ROLEADMIN'])).toEqual(false);
  });
});
