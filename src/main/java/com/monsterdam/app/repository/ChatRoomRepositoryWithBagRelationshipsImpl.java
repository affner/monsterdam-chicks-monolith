...
public class ChatRoomRepositoryWithBagRelationshipsImpl implements ChatRoomRepositoryWithBagRelationships {
    ...
    public Optional<ChatRoom> fetchBagRelationships(Optional<ChatRoom> chatRoom) {
        return chatRoom.map(this::fetchParticipants);
    }
    ...
    public List<ChatRoom> fetchBagRelationships(List<ChatRoom> chatRooms) {
        return Optional.of(chatRooms).map(this::fetchParticipants).orElse(Collections.emptyList());
    }

    ChatRoom fetchParticipants(ChatRoom result) {
        return entityManager
            .createQuery(
                "select chatRoom from ChatRoom chatRoom left join fetch chatRoom.participants where chatRoom.id = :id",
                ChatRoom.class
            )
    ...
    List<ChatRoom> fetchParticipants(List<ChatRoom> chatRooms) {
        ...
        List<ChatRoom> result = entityManager
            .createQuery(
                "select chatRoom from ChatRoom chatRoom left join fetch chatRoom.participants where chatRoom in :chatRooms",
                ChatRoom.class
            )
