 ...
 */
@RestController
@RequestMapping("/api/crud/single-photos")
public class SinglePhotoResource {
    ...
    public ResponseEntity<SinglePhotoDTO> createSinglePhoto(@Valid @RequestBody SinglePhotoDTO singlePhotoDTO) throws URISyntaxException {
        ...
        singlePhotoDTO = singlePhotoService.save(singlePhotoDTO);
        return ResponseEntity.created(new URI("/api/crud/single-photos/" + singlePhotoDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, singlePhotoDTO.getId().toString()))
            .body(singlePhotoDTO);
    }
    ...
    public ResponseEntity<SinglePhotoDTO> updateSinglePhoto(
        ...
        singlePhotoDTO = singlePhotoService.update(singlePhotoDTO);
