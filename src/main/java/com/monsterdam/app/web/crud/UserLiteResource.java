 ...
 */
@RestController
@RequestMapping("/api/crud/user-lites")
public class UserLiteResource {
    ...
    public ResponseEntity<UserLiteDTO> createUserLite(@Valid @RequestBody UserLiteDTO userLiteDTO) throws URISyntaxException {
        ...
        userLiteDTO = userLiteService.save(userLiteDTO);
        return ResponseEntity.created(new URI("/api/crud/user-lites/" + userLiteDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, userLiteDTO.getId().toString()))
            .body(userLiteDTO);
    }
    ...
    public ResponseEntity<UserLiteDTO> updateUserLite(
        ...
        userLiteDTO = userLiteService.update(userLiteDTO);
