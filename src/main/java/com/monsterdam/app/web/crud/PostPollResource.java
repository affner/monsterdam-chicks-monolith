 ...
 */
@RestController
@RequestMapping("/api/crud/post-polls")
public class PostPollResource {
    ...
    public ResponseEntity<PostPollDTO> createPostPoll(@Valid @RequestBody PostPollDTO postPollDTO) throws URISyntaxException {
        ...
        postPollDTO = postPollService.save(postPollDTO);
        return ResponseEntity.created(new URI("/api/crud/post-polls/" + postPollDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, postPollDTO.getId().toString()))
            .body(postPollDTO);
    }
    ...
    public ResponseEntity<PostPollDTO> updatePostPoll(
        ...
        postPollDTO = postPollService.update(postPollDTO);
