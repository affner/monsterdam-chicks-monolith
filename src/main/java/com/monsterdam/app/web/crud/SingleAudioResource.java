 ...
 */
@RestController
@RequestMapping("/api/crud/single-audios")
public class SingleAudioResource {
    ...
    public ResponseEntity<SingleAudioDTO> createSingleAudio(@Valid @RequestBody SingleAudioDTO singleAudioDTO) throws URISyntaxException {
        ...
        singleAudioDTO = singleAudioService.save(singleAudioDTO);
        return ResponseEntity.created(new URI("/api/crud/single-audios/" + singleAudioDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, singleAudioDTO.getId().toString()))
            .body(singleAudioDTO);
    }
    ...
    public ResponseEntity<SingleAudioDTO> updateSingleAudio(
        ...
        singleAudioDTO = singleAudioService.update(singleAudioDTO);
