 ...
 */
@RestController
@RequestMapping("/api/crud/video-stories")
public class VideoStoryResource {
    ...
    public ResponseEntity<VideoStoryDTO> createVideoStory(@Valid @RequestBody VideoStoryDTO videoStoryDTO) throws URISyntaxException {
        ...
        videoStoryDTO = videoStoryService.save(videoStoryDTO);
        return ResponseEntity.created(new URI("/api/crud/video-stories/" + videoStoryDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, videoStoryDTO.getId().toString()))
            .body(videoStoryDTO);
    }
    ...
    public ResponseEntity<VideoStoryDTO> updateVideoStory(
        ...
        videoStoryDTO = videoStoryService.update(videoStoryDTO);
