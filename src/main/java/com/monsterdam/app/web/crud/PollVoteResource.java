...
/**
 * REST controller for managing {@link com.monsterdam.app.domain.PollVote}.
 */
@RestController
@RequestMapping("/api/crud/poll-votes")
public class PollVoteResource {

    private static final Logger LOG = LoggerFactory.getLogger(PollVoteResource.class);
    ...
    public ResponseEntity<PollVoteDTO> createPollVote(@Valid @RequestBody PollVoteDTO pollVoteDTO) throws URISyntaxException {
        ...
        pollVoteDTO = pollVoteService.save(pollVoteDTO);
        return ResponseEntity.created(new URI("/api/crud/poll-votes/" + pollVoteDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, pollVoteDTO.getId().toString()))
            .body(pollVoteDTO);
    }
    ...
    public ResponseEntity<PollVoteDTO> updatePollVote(
        ...
        pollVoteDTO = pollVoteService.update(pollVoteDTO);
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<PollVoteDTO>> getAllPollVotes(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        LOG.debug("REST request to get a page of PollVotes");
        Page<PollVoteDTO> page = pollVoteService.findAll(pageable);
