...
/**
 * REST controller for managing {@link com.monsterdam.app.domain.PollOption}.
 */
@RestController
@RequestMapping("/api/crud/poll-options")
public class PollOptionResource {

    private static final Logger LOG = LoggerFactory.getLogger(PollOptionResource.class);
    ...
    public ResponseEntity<PollOptionDTO> createPollOption(@Valid @RequestBody PollOptionDTO pollOptionDTO) throws URISyntaxException {
        ...
        pollOptionDTO = pollOptionService.save(pollOptionDTO);
        return ResponseEntity.created(new URI("/api/crud/poll-options/" + pollOptionDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, pollOptionDTO.getId().toString()))
            .body(pollOptionDTO);
    }
    ...
    public ResponseEntity<PollOptionDTO> updatePollOption(
        ...
        pollOptionDTO = pollOptionService.update(pollOptionDTO);
