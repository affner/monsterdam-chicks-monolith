 ...
 */
@RestController
@RequestMapping("/api/crud/like-marks")
public class LikeMarkResource {
    ...
    public ResponseEntity<LikeMarkDTO> createLikeMark(@Valid @RequestBody LikeMarkDTO likeMarkDTO) throws URISyntaxException {
        ...
        likeMarkDTO = likeMarkService.save(likeMarkDTO);
        return ResponseEntity.created(new URI("/api/crud/like-marks/" + likeMarkDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, likeMarkDTO.getId().toString()))
            .body(likeMarkDTO);
    }
    ...
    public ResponseEntity<LikeMarkDTO> updateLikeMark(
        ...
        likeMarkDTO = likeMarkService.update(likeMarkDTO);
