...
describe('Payment e2e test', () => {
  ...
  const paymentSample = { amount: 1075.67, paymentDate: '2025-12-10T02:54:08.879Z', paymentStatus: 'DECLINED', viewerId: 16457 };
  ...
  describe('new Payment page', () => {
    ...
    it('should create an instance of Payment', () => {
      cy.get(`[data-cy="amount"]`).type('322.05');
      cy.get(`[data-cy="amount"]`).should('have.value', '322.05');

      cy.get(`[data-cy="paymentDate"]`).type('2025-12-10T02:47');
      cy.get(`[data-cy="paymentDate"]`).blur();
      cy.get(`[data-cy="paymentDate"]`).should('have.value', '2025-12-10T02:47');

      cy.get(`[data-cy="paymentStatus"]`).select('CANCELED');

      cy.get(`[data-cy="paymentReference"]`).type('a regarding');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'a regarding');

      cy.get(`[data-cy="cloudTransactionId"]`).type('collaborate representation');
      cy.get(`[data-cy="cloudTransactionId"]`).should('have.value', 'collaborate representation');

      cy.get(`[data-cy="viewerId"]`).type('27992');
      cy.get(`[data-cy="viewerId"]`).should('have.value', '27992');
