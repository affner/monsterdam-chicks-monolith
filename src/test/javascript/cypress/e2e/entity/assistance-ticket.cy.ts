...
describe('AssistanceTicket e2e test', () => {
  ...
  const assistanceTicketSample = {
    ...
    status: 'OPEN',
    type: 'OTHER',
    createdDate: '2025-12-10T07:52:16.063Z',
    userId: 26804,
  };
  ...
  describe('new AssistanceTicket page', () => {
    ...
    it('should create an instance of AssistanceTicket', () => {
      cy.get(`[data-cy="subject"]`).type('suspiciously');
      cy.get(`[data-cy="subject"]`).should('have.value', 'suspiciously');

      cy.get(`[data-cy="description"]`).type('customise');
      cy.get(`[data-cy="description"]`).should('have.value', 'customise');

      cy.get(`[data-cy="status"]`).select('CLOSED');

      cy.get(`[data-cy="type"]`).select('DOCUMENT_VERIFICATION');

      cy.get(`[data-cy="openedAt"]`).type('2025-12-09T19:17');
      cy.get(`[data-cy="openedAt"]`).blur();
      cy.get(`[data-cy="openedAt"]`).should('have.value', '2025-12-09T19:17');

      cy.get(`[data-cy="closedAt"]`).type('2025-12-10T07:24');
      cy.get(`[data-cy="closedAt"]`).blur();
      cy.get(`[data-cy="closedAt"]`).should('have.value', '2025-12-10T07:24');

      cy.get(`[data-cy="comments"]`).type('depute failing rust');
      cy.get(`[data-cy="comments"]`).should('have.value', 'depute failing rust');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-10T03:16');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-10T03:16');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2025-12-09T12:21');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2025-12-09T12:21');

      cy.get(`[data-cy="createdBy"]`).type('huzzah now phew');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'huzzah now phew');

      cy.get(`[data-cy="lastModifiedBy"]`).type('orientate likewise');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'orientate likewise');

      cy.get(`[data-cy="userId"]`).type('29452');
      cy.get(`[data-cy="userId"]`).should('have.value', '29452');
