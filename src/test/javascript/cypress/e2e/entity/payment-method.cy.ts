...
describe('PaymentMethod e2e test', () => {
  ...
  const paymentMethodSample = { methodName: 'idolized till blah', tokenText: 'amid fully manner', createdDate: '2025-12-16T00:06:48.936Z' };
  ...
  describe('new PaymentMethod page', () => {
    ...
    it('should create an instance of PaymentMethod', () => {
      cy.get(`[data-cy="methodName"]`).type('ugh whether valley');
      cy.get(`[data-cy="methodName"]`).should('have.value', 'ugh whether valley');

      cy.get(`[data-cy="tokenText"]`).type('um festival voluminous');
      cy.get(`[data-cy="tokenText"]`).should('have.value', 'um festival voluminous');

      cy.get(`[data-cy="expirationDate"]`).type('2025-12-15');
      cy.get(`[data-cy="expirationDate"]`).blur();
      cy.get(`[data-cy="expirationDate"]`).should('have.value', '2025-12-15');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-15T22:33');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-15T22:33');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2025-12-15T21:26');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2025-12-15T21:26');

      cy.get(`[data-cy="createdBy"]`).type('scope roasted after');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'scope roasted after');

      cy.get(`[data-cy="lastModifiedBy"]`).type('eek');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'eek');

      cy.get(`[data-cy="deletedDate"]`).type('2025-12-15T21:18');
      cy.get(`[data-cy="deletedDate"]`).blur();
      cy.get(`[data-cy="deletedDate"]`).should('have.value', '2025-12-15T21:18');
