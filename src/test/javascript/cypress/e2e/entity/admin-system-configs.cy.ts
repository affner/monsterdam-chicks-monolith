...
describe('AdminSystemConfigs e2e test', () => {
  ...
  const adminSystemConfigsSample = {
    configKey: 'among oxidize whoa',
    configValue: 'perfectly woot',
    createdDate: '2025-12-15T18:55:09.131Z',
  };
  ...
  describe('new AdminSystemConfigs page', () => {
    ...
    it('should create an instance of AdminSystemConfigs', () => {
      cy.get(`[data-cy="configKey"]`).type('boo');
      cy.get(`[data-cy="configKey"]`).should('have.value', 'boo');

      cy.get(`[data-cy="configValue"]`).type('gah scaffold');
      cy.get(`[data-cy="configValue"]`).should('have.value', 'gah scaffold');

      cy.get(`[data-cy="description"]`).type('that midst');
      cy.get(`[data-cy="description"]`).should('have.value', 'that midst');

      cy.get(`[data-cy="configValueType"]`).select('INTEGER');

      cy.get(`[data-cy="configCategory"]`).select('PAYMENT');

      cy.setFieldImageAsBytesOfEntity('configFile', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-15T13:19');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-15T13:19');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2025-12-15T11:24');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2025-12-15T11:24');

      cy.get(`[data-cy="createdBy"]`).type('furthermore deprave');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'furthermore deprave');

      cy.get(`[data-cy="lastModifiedBy"]`).type('kowtow buzzing');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'kowtow buzzing');

      cy.get(`[data-cy="deletedDate"]`).type('2025-12-16T05:06');
      cy.get(`[data-cy="deletedDate"]`).blur();
      cy.get(`[data-cy="deletedDate"]`).should('have.value', '2025-12-16T05:06');
