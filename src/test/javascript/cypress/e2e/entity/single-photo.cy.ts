...
describe('SinglePhoto e2e test', () => {
  ...
  const singlePhotoSample = {
    thumbnailS3Key: 'instead',
    contentS3Key: 'inasmuch critical',
    createdDate: '2025-12-09T12:56:39.144Z',
    isDeleted: true,
    creatorId: 24282,
  };
  ...
  describe('new SinglePhoto page', () => {
    ...
    it('should create an instance of SinglePhoto', () => {
      cy.get(`[data-cy="thumbnailS3Key"]`).type('victoriously slushy');
      cy.get(`[data-cy="thumbnailS3Key"]`).should('have.value', 'victoriously slushy');

      cy.get(`[data-cy="contentS3Key"]`).type('beside modulo times');
      cy.get(`[data-cy="contentS3Key"]`).should('have.value', 'beside modulo times');

      cy.get(`[data-cy="likeCount"]`).type('9802');
      cy.get(`[data-cy="likeCount"]`).should('have.value', '9802');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-10T06:45');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-10T06:45');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2025-12-09T17:04');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2025-12-09T17:04');

      cy.get(`[data-cy="createdBy"]`).type('guilt sadly');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'guilt sadly');

      cy.get(`[data-cy="lastModifiedBy"]`).type('petal');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'petal');

      cy.get(`[data-cy="isDeleted"]`).should('not.be.checked');
      cy.get(`[data-cy="isDeleted"]`).click();
      cy.get(`[data-cy="isDeleted"]`).should('be.checked');

      cy.get(`[data-cy="creatorId"]`).type('11260');
      cy.get(`[data-cy="creatorId"]`).should('have.value', '11260');
