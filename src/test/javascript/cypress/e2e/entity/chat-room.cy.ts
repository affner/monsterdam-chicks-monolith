...
describe('ChatRoom e2e test', () => {
  ...
  const chatRoomSample = { createdDate: '2025-12-10T02:00:03.931Z', isDeleted: false };
  ...
  describe('new ChatRoom page', () => {
    ...
    it('should create an instance of ChatRoom', () => {
      cy.get(`[data-cy="lastAction"]`).type('youthfully provided annex');
      cy.get(`[data-cy="lastAction"]`).should('have.value', 'youthfully provided annex');

      cy.get(`[data-cy="lastConnectionDate"]`).type('2025-12-09T10:13');
      cy.get(`[data-cy="lastConnectionDate"]`).blur();
      cy.get(`[data-cy="lastConnectionDate"]`).should('have.value', '2025-12-09T10:13');

      cy.get(`[data-cy="muted"]`).should('not.be.checked');
      cy.get(`[data-cy="muted"]`).click();
      cy.get(`[data-cy="muted"]`).should('be.checked');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-10T06:21');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-10T06:21');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2025-12-10T05:28');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2025-12-10T05:28');

      cy.get(`[data-cy="createdBy"]`).type('to');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'to');

      cy.get(`[data-cy="lastModifiedBy"]`).type('consequently blah while');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'consequently blah while');

      cy.get(`[data-cy="isDeleted"]`).should('not.be.checked');
      cy.get(`[data-cy="isDeleted"]`).click();
      cy.get(`[data-cy="isDeleted"]`).should('be.checked');

      cy.get(`[data-cy="participantId"]`).type('26747');
      cy.get(`[data-cy="participantId"]`).should('have.value', '26747');
