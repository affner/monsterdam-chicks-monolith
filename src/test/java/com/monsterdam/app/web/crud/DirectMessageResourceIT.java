...
class DirectMessageResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/crud/direct-messages";
    ...
    public static DirectMessage createEntity(EntityManager em) {
        ...
        // Add required entity
        ChatRoom chatRoom;
        if (TestUtil.findAll(em, ChatRoom.class).isEmpty()) {
            chatRoom = ChatRoomResourceIT.createEntity();
            em.persist(chatRoom);
            em.flush();
        } else {
            chatRoom = TestUtil.findAll(em, ChatRoom.class).get(0);
        }
        directMessage.setChatRoom(chatRoom);
        // Add required entity
        UserLite userLite;
        ...
        directMessage.setSender(userLite);
    ...
    public static DirectMessage createUpdatedEntity(EntityManager em) {
        DirectMessage updatedDirectMessage = new DirectMessage()
        ...
        ChatRoom chatRoom;
        if (TestUtil.findAll(em, ChatRoom.class).isEmpty()) {
            chatRoom = ChatRoomResourceIT.createUpdatedEntity();
            em.persist(chatRoom);
            em.flush();
        } else {
            chatRoom = TestUtil.findAll(em, ChatRoom.class).get(0);
        }
        updatedDirectMessage.setChatRoom(chatRoom);
        // Add required entity
        UserLite userLite;
        if (TestUtil.findAll(em, UserLite.class).isEmpty()) {
        ...
        updatedDirectMessage.setSender(userLite);
