...
class PollVoteResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/crud/poll-votes";
    ...
    public static PollVote createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, PollOption.class).isEmpty()) {
            pollOption = PollOptionResourceIT.createEntity();
    ...
    public static PollVote createUpdatedEntity(EntityManager em) {
        PollVote updatedPollVote = new PollVote().createdDate(UPDATED_CREATED_DATE);
        // Add required entity
        PollOption pollOption;
        if (TestUtil.findAll(em, PollOption.class).isEmpty()) {
            pollOption = PollOptionResourceIT.createUpdatedEntity();
            em.persist(pollOption);
            em.flush();
        } else {
            pollOption = TestUtil.findAll(em, PollOption.class).get(0);
        }
        updatedPollVote.setPollOption(pollOption);
        // Add required entity
        UserLite userLite;
        if (TestUtil.findAll(em, UserLite.class).isEmpty()) {
        ...
        updatedPollVote.setVoter(userLite);
